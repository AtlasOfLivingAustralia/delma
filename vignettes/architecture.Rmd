...
the `xml2` package. All new parsers in `paperbark` are built to plug into the formats 
defined by `xml2`.
...
new, bespoke S3 classes for the `paperbark` package, as it would be nice to parse
_any_ character, tibble, list or xml_document, not just those that are created 
using `paperbark`.

To avoid these problems, `paperbark` uses the `as_eml_` prefix for all transformations,
