...
When importing from markdown, we use `read_elm_chr()`:
...
Conversely, when reading `.xml` files, we use `read_elm_xml()`. Note that this
function can also accept an online address, as well as a file:

```{r}
read_elm_xml("https://collections.ala.org.au/ws/eml/dr368")
```

The corresponding write functions are `write_elm_chr()` and `write_elm_xml()`. 
