...
To avoid these problems, `elm` uses the `as_md_` prefix for all transformations,
...
When importing from markdown, we use `read_md_chr()`:
...
Conversely, when reading `.xml` files, we use `read_md_xml()`. Note that this
function can also accept an online address, as well as a file:

```{r}
read_md_xml("https://collections.ala.org.au/ws/eml/dr368")
```

The corresponding write functions are `write_md_chr()` and `write_md_xml()`. 
Both accept objects of all classes supported by `elm`, so it is possible to 
chain read and write functions together without any intermediate stages:
...
system.file("example_data", 
  ...
  readLines() |> # from `base`
  as_md_xml() |> # from `elm`
  write_xml("output.xml") # from `xml2`
```
