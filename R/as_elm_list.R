...
#' @export
as_elm_list <- function(x, ...){
  UseMethod("as_elm_list")
}

#' @rdname as_elm_list
#' @order 2
#' @exportS3Method elm::as_elm_list
as_elm_list.character <- function(x, ...){
  x |>
    parse_chr_to_tibble() |>
...
#' @order 5
#' @exportS3Method elm::as_elm_list
as_elm_list.xml_document <- function(x, ...){
  parse_xml_to_list(x)   
}
